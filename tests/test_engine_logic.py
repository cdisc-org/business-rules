...
class EngineTests(TestCase):
    ...
    def test_check_all_conditions_series_results_single_condition(self, mock_check_condition):
        conditions = {'all': [{'thing1': ''}, {'thing2': ''}]}
    ...
    def test_check_all_conditions_series_results_multiple_conditions(self, mock_check_condition):
        condition_results = {
            1: pd.Series([True, False, True, True, ]),
            2: pd.Series([True, False, False, True, ]),
        }
        conditions = {'all': [{'id': 1}, {'id': 2}]}
        mock_check_condition.side_effect = lambda condition, defined_variables: condition_results[condition["id"]]
        ...
        variables = DatasetVariables()
        result: pd.Series = engine.check_conditions_recursively(conditions, variables)
        self.assertTrue(result.equals(pd.Series([True, False, False, True, ])))

    @patch.object(engine, 'check_condition', return_value=pd.Series([True, True, False]))
    def test_check_not_all_conditions_series_results_single_condition(self, mock_check_condition):
        conditions = {'not': {'all': [{'thing1': ''}, {'thing2': ''}]}}
    ...
    def test_check_not_all_conditions_series_results_multiple_conditions(self, mock_check_condition):
        condition_results = {
            1: pd.Series([False, False, True, True]),
            2: pd.Series([False, False, True, False]),
        }
        conditions = {'not': {'all': [{'id': 1}, {'id': 2}]}}
        mock_check_condition.side_effect = lambda condition, defined_variables: condition_results[condition["id"]]
        ...
        variables = DatasetVariables()
        result: pd.Series = engine.check_conditions_recursively(conditions, variables)
        self.assertTrue(result.equals(pd.Series([True, True, False, True, ])))
    ...
    @patch.object(engine, 'check_condition', return_value=pd.Series([True, True, True, ]))
    def test_check_any_condition_series_results_single_condition(self, mock_check_condition):
        conditions = {'any': [{'thing1': ''}, {'thing2': ''}]}
    ...
    @patch.object(engine, 'check_condition')
    def test_check_any_condition_series_results_multiple_conditions(self, mock_check_condition):
        condition_results = {
            1: pd.Series([False, False, True, False]),
            2: pd.Series([True, False, False, False]),
        }
        conditions = {'any': [{'id': 1}, {'id': 2}]}
        mock_check_condition.side_effect = lambda condition, defined_variables: condition_results[condition["id"]]
        ...
        variables = DatasetVariables()
        result: pd.Series = engine.check_conditions_recursively(conditions, variables)
        self.assertTrue(result.equals(pd.Series([True, False, True, False])))

    @patch.object(engine, 'check_condition', return_value=pd.Series([True, True, True, False, ]))
    def test_check_not_any_condition_series_results_single_condition(self, mock_check_condition):
        conditions = {'not': {'any': [{'thing1': ''}, ]}}
        ...
        variables = DatasetVariables()
        result: pd.Series = engine.check_conditions_recursively(conditions, variables)
        self.assertTrue(result.equals(pd.Series([False, False, False, True, ])))

    @patch.object(engine, 'check_condition')
    def test_check_not_any_condition_series_results_multiple_conditions(self, mock_check_condition):
        condition_results = {
            1: pd.Series([False, False, True, False]),
            2: pd.Series([True, False, False, False]),
        }
        conditions = {'not': {'any': [{'id': 1}, {'id': 2}]}}
        mock_check_condition.side_effect = lambda condition, defined_variables: condition_results[condition["id"]]
        ...
        variables = DatasetVariables()
        result: pd.Series = engine.check_conditions_recursively(conditions, variables)
        self.assertTrue(result.equals(pd.Series([False, True, False, True])))
