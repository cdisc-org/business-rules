...
class DataframeOperatorTests(TestCase):
    ...
    def test_is_ordered_set(self):
        df = pandas.DataFrame.from_dict( {"USUBJID": [1,2,1,2], "ECSEQ": [1,1,2,2] })
        self.assertTrue(DataframeType({"value": df}).is_ordered_set({"target" : "ECSEQ", "comparator": "USUBJID"}))
        self.assertTrue(DataframeType({"value":df, "column_prefix_map": {"--": "EC"}}).is_ordered_set({"target" : "--SEQ", "comparator": "USUBJID"}))
        
        df2 = pandas.DataFrame.from_dict( {"USUBJID": [1,2,1,2], "ECSEQ": [3,1,2,2] })
        self.assertFalse(DataframeType({"value": df2}).is_ordered_set({"target" : "ECSEQ", "comparator": "USUBJID"}))
        self.assertFalse(DataframeType({"value":df2, "column_prefix_map": {"--": "EC"}}).is_ordered_set({"target" : "--SEQ", "comparator": "USUBJID"}))
        
    def test_is_not_ordered_set(self):
        df = pandas.DataFrame.from_dict( {"USUBJID": [1,2,1,2], "ECSEQ": [3,1,2,2] })
        self.assertTrue(DataframeType({"value": df}).is_not_ordered_set({"target" : "ECSEQ", "comparator": "USUBJID"}))
        self.assertTrue(DataframeType({"value":df, "column_prefix_map": {"--": "EC"}}).is_not_ordered_set({"target" : "--SEQ", "comparator": "USUBJID"}))
        
        df2 = pandas.DataFrame.from_dict( {"USUBJID": [1,2,1,2], "ECSEQ": [1,1,2,2] })
        self.assertFalse(DataframeType({"value": df2}).is_not_ordered_set({"target" : "ECSEQ", "comparator": "USUBJID"}))
        self.assertFalse(DataframeType({"value":df2, "column_prefix_map": {"--": "EC"}}).is_not_ordered_set({"target" : "--SEQ", "comparator": "USUBJID"}))
